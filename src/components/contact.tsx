  ...
  <div className="container mx-auto max-w-6xl px-4 sm:px-6">
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      {contactData.map((item) => (
        <div 
          key={item.id} 
          className="group bg-linear-to-br from-gray-900 to-black border border-gray-800 rounded-2xl p-8 transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-blue-500/10 hover:border-blue-500/20 relative overflow-hidden"
        >
          ...
          <div className="flex items-start mb-6">
            ...
            <div className="flex-1">
              <h3 className="ms-5 text-white text-[22px] font-bold leading-tight mb-4 group-hover:text-blue-100 transition-colors duration-300">
                {item.title}
              </h3>
              <div className="space-y-3">
                {item.links.map((link, index) => (
                  <a
                    key={index}
                    href={link.href}
                    className="flex items-center text-gray-400 text-[16px] leading-relaxed hover:text-white transition-all duration-300 group/link hover:translate-x-1"
